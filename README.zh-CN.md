...
Windows 自带 Win+H 语音输入，讯飞也免费——为什么还要言笔？**免费工具转写的是你说的话，言笔上屏的是你想写的字。**

![言笔 vs 免费方案](docs/images/vs-free.png)

- **口语进去，书面语出来**：语气词、重复、跑题自动整理成通顺书面语（DeepSeek），还按应用自动切换语气（微信口语感 / 文档书面化）——免费工具贴的是原文
- **热词闭环**：你的行业术语直传识别引擎 + 整理时保护不改写，越用越准
- **语音模板 / 语音命令**：说「发X」上屏一整段固定内容，说「记一下」教它一个词
- **自己的 API Key**：无账号、无广告、不绑消费级云生态
- 老实说代价：言笔按量付费 + 要花约 10 分钟开通 API；只图免费省事，Win+H 或讯飞就够用
